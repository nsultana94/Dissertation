...
# initialiser.load_state_dict(torch.load(f'{DATA_URL}Models/2layers_finetuned_3kernels.pt'))

# checkpoint = torch.load(f'{DATA_URL}Models/kernel.pt')
# initialiser.load_state_dict(checkpoint['model_state_dict'])
# torch.save(initialiser.state_dict(), f'{DATA_URL}Models/U-net/4layers_finetuned.pt')
...
# new_model.load_state_dict(torch.load(f'{DATA_URL}Models/initialiser3frames_finetuned_withpseudolabels.pt'))
...
# images, masks = testset[40]
# initialiser.eval()
# # logits, cell_states = new_model(images.unsqueeze(0).to(DEVICE))
...
# plt.savefig(f'prediction_nextframe_2.png')
...
def calculate_iou(gt_mask, pred_mask, num_classes):
    ...
    for c in range(num_classes):
        ...
        gt_binary = np.asarray(gt_mask == c, dtype=np.int32)
        pred_binary = np.asarray(pred_mask == c, dtype=np.int32)

        # compute intersection and union
        intersection = np.logical_and(gt_binary, pred_binary)
        union = np.logical_or(gt_binary, pred_binary)

        # count number of 1s in intersection and union
        intersection_count = np.sum(intersection)
        union_count = np.sum(union)

        # compute IoU for class c
        iou_c = intersection_count / union_count
        iou_per_class.append(iou_c)
        
    return iou_per_class

def calculate_mean_iou(gt_masks, pred_masks, num_classes):
    iou_per_class_all_images = []
    for i in range(len(gt_masks)):
        iou_per_class = calculate_iou(gt_masks[i], pred_masks[i], num_classes)
        iou_per_class_all_images.append(iou_per_class)
    
    mean_iou_per_class = np.mean(iou_per_class_all_images, axis=0)
...
for idx in range (0, len(testset)):
  ...
  with torch.no_grad():
    ...
    # features = encoder(images[2].unsqueeze(0).to(DEVICE))
    ...
    # h0_similarity = hidden_loss(h_next, feature_vector)
    # c0_similarity = hidden_loss(c_next, c_original)
    # print(h0_similarity, c0_similarity)
